...
from keyplus.application.errors import EntryNotFoundError


def build_view_entry(service, entry_id, on_back, on_edit, on_delete):
    """
    ...
    edit_btn.clicked.connect(lambda: on_edit(entry_id))
    top_bar.addWidget(edit_btn)
    delete_btn = QPushButton("Delete")
    delete_btn.setStyleSheet("color: #ef4444;")
    delete_btn.clicked.connect(lambda: on_delete(entry_id))
    ...
    if not entry:
        ...
        error_box.setStyleSheet(
            "background-color: #2d1e24; border: 1px solid #7f1d1d; border-radius: 8px;"
        )
    ...
    created = QLabel(f"Created: {entry.created_at}")
    ...
    if entry.updated_at and entry.updated_at != entry.created_at:
        updated = QLabel(f"Last Modified: {entry.updated_at}")
