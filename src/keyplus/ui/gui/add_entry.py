# KeyPlus GUI add/edit entry dialog
...
class AddEntryDialog(QDialog):
    def __init__(self, service, parent=None, entry=None):
        ...
        # --- HEADER ---
        title = QLabel("Edit Entry" if entry else "Add New Entry")
        ...
        if entry:
            self.domain_input.setText(entry.domain)
        ...
        add_btn = QPushButton("Save Changes" if entry else "Save Entry")
    ...
    def _handle_add(self):
        ...
        if not name or not domain or (not password and self.entry is None):
            self.error.setText("⚠️ Name, domain, and a new-entry password are required.")
        ...
        try:
            if self.entry is None:
                entry = self.service.add_entry(EntryDraft(name, domain, password))
            else:
                entry = self.service.update_entry(
                    self.entry.id,
                    EntryChanges(name=name, domain=domain, password=password or None),
                )
