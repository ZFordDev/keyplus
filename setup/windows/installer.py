...
class ModernInstallerUI:
    ...
    def apply_theme_styles(self):
        ...
        self.style.configure(
            "Sub.TLabel", font=("Segoe UI", 10, "bold"), foreground="#334155"
        )
        self.style.configure("Info.TLabel", font=("Segoe UI", 9), foreground="#64748b")

        self.style.configure(
            "TEntry", fieldbackground="#f1f5f9", bordercolor="#cbd5e1", padding=6
        )

        self.style.configure(
            "TCheckbutton", font=("Segoe UI", 10), background="#ffffff"
        )
    ...
    def assemble_ui(self):
        ...
        ttk.Label(
            header_frame,
            text=f"Install {self.engine.app_name} Package",
            style="Header.TLabel",
        ...
        self.content_frame = ttk.Frame(
            self.root, padding="28 24 28 24"
        )  # Increased bottom padding
        self.content_frame.pack(fill="both", expand=True)

        ttk.Label(
            self.content_frame, text="Where should KeyPlus live?", style="Sub.TLabel"
        ...
        ttk.Label(
            self.content_frame, text="Extra Setup Options", style="Sub.TLabel"
        ).pack(anchor="w", pady=(0, 8))

        ttk.Checkbutton(
            self.content_frame,
            text="Add KeyPlus to the command line (so you can type 'keyplus' and it just works)",
            variable=self.add_to_path_var,
        ).pack(anchor="w", pady=5)
        ttk.Checkbutton(
            self.content_frame,
            text="Put a shiny KeyPlus icon on your desktop (for the click‑first people)",
            variable=self.create_shortcut_var,
        ).pack(anchor="w", pady=5)
        ttk.Checkbutton(
            self.content_frame,
            text="Add an uninstaller (for when you decide to break my heart and remove it)",
            variable=self.create_uninstaller_var,
        ).pack(anchor="w", pady=5)

        ttk.Label(
            self.content_frame,
            text="KeyPlus will be installed inside your LocalAppData folder. Nice and tidy.",
            style="Info.TLabel",
        ...
        self.status_label = ttk.Label(
            self.status_frame,
            text="Installing KeyPlus… please hold your excitement.",
            style="Info.TLabel",
        )
    ...
    def run_engine_deployment(self):
        ...
        except Exception as e:  # noqa: BLE001 - installer boundary reports deployment failures
            self.progress_bar.stop()
